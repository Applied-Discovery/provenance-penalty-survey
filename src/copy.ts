...
// Registered wording. `noun` is set only for a domain with `label_noun` on ("This answer was ..."), where the artifact
// shows something the label must not cover, such as a human-asked question above the answer.
const labelSubject = (noun?: string) => (noun ? `This ${noun}` : 'This');
export function aiLabel(noun?: string): string { return `${labelSubject(noun)} was generated by an AI system.`; }
export function humanLabel(verb: string, noun?: string): string { return `${labelSubject(noun)} was ${verb} by a person.`; }
