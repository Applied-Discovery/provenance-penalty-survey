...
export const manifestSchema = z.object({
  ...
  // failed checks after which a session goes to attention_redirect. Prolific allows rejecting on attention only after
  // two failed checks in a study of 5 minutes or longer, so 2 unless the study is shorter
  attention_reject_failures: z.number().int().min(1).default(2),
  ...
  if (m.attention_redirect && m.attention_checks < m.attention_reject_failures)   // it could never fire
    ctx.addIssue({ code: 'custom', path: ['attention_redirect'], message:
      `attention_redirect needs at least ${m.attention_reject_failures} attention checks, since it fires only when attention_reject_failures of them fail (got ${m.attention_checks})` });
