...
test('label_noun names the noun in every provenance label a session shows', async ({ page }) => {
  await interceptDataPipe(page);
  await patchManifest(page, { stem_noun: 'answer', label_noun: true });
  await page.goto('/domains/example_markdown/?SESSION_ID=e2e-label-noun');
  await page.getByRole('button', { name: 'I agree' }).click();
  const labels: string[] = [];
  for (let i = 0; i < 4; i++) {                                    // attention check + 2 labeled + 1 unlabeled
    await expect(page.locator('.artifact-markdown')).toBeVisible();
    const label = page.locator('p.label');
    if (await label.count() && !(await page.getByText('attention check').isVisible())) labels.push(await label.innerText());
    if (await page.getByText('attention check').isVisible()) await page.locator('button.rating-btn').nth(2).click(); else await page.locator('button.rating-btn').first().click();
    if (await page.getByRole('button', { name: 'Continue' }).isVisible()) await page.getByRole('button', { name: 'Continue' }).click();
    if (await page.getByRole('button', { name: 'A person' }).isVisible()) await page.getByRole('button', { name: 'A person' }).click();
  }
  expect(labels.sort()).toEqual(['This answer was generated by an AI system.', 'This answer was written by a person.']);
});
