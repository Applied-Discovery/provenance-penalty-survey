...
test('scaffold honours explicit session shape, class and minutes, and omits human_verb when not given', () => {
  ...
  expect(manifestOf('code_review')).toMatchObject({ artifact_type: 'code', evaluator_class: 'expert', expected_minutes: '10', labeled_artifacts_per_session: 2, unlabeled_per_session: 1, attention_checks: 0 });
  expect(manifestOf('code_review')).not.toHaveProperty('human_verb');
});
test('scaffold accepts a mixed-language code pool and refuses one with an unknown extension, writing nothing', () => {
  ...
  expect(manifestOf('code_mixed')).toMatchObject({ artifact_type: 'code' });
