...
// Failed attention checks: the platform's separate completion code, when the manifest names one, once a single check
// has failed. The surveys are under 5 minutes, where Prolific allows rejecting on one failure.
...
test('finalRedirect: the attention URL once one check has failed, else the completion redirect', () => {
  const c = { ...ctx, redirect: 'https://p.test/done' };
  ...
  expect(finalRedirect(ma, c, [failedCheck, passedCheck])).toBe('https://p.test/attn');
  ...
  expect(finalRedirect(ma, ctx, [failedCheck, passedCheck])).toBe('https://p.test/attn');   // even without a completion redirect
});
test('finalRedirect: with a single check, its failure is enough', () => {
  const m1 = validateManifest({ ...m, attention_checks: 1, attention_redirect: 'https://p.test/attn' } as any);
  expect(finalRedirect(m1, ctx, [failedCheck])).toBe('https://p.test/attn');
  expect(finalRedirect(m1, ctx, [passedCheck])).toBeUndefined();
});
