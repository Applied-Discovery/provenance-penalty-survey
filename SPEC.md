...
```json
{
    ...
    label_noun?: false, // default false. When true, both labels name the stem noun: "This answer was written by a person." / "This answer was generated by an AI system." For an artifact that shows something the label must not cover, such as a human-asked question above the answer. Off, the labels keep the registered wording
    expected_minutes?: "5-10", // default: "5-10". Shown in consent as "about 5-10 minutes"
    attention_checks?: 1, // default: 1
    attention_reject_failures?: 2, // default: 2. Failed checks that send a session to attention_redirect; 2 is Prolific's rejection threshold for 5+ minute studies, a shorter study may set 1
    ...
    attention_redirect?: "", // URL to redirect to instead when at least attention_reject_failures attention checks were answered wrongly; needs attention_checks >= attention_reject_failures; a failed prescreen still wins
